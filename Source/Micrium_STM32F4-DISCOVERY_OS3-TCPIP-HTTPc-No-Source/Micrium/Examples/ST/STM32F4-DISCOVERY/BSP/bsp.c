...
#define  BSP_BIT_RCC_PLLCFGR_PLLN              288u
#define  BSP_BIT_RCC_PLLCFGR_PLLP                2u
#define  BSP_BIT_RCC_PLLCFGR_PLLQ                6u
...
void  BSP_Init (void)
{
    ...
    RCC_DeInit();
    RCC_HSEConfig(RCC_HSE_ON);                                  /* HSE = 8 MHz ext. crystal.                            */
    RCC_WaitForHSEStartUp();
    
    RCC_HCLKConfig(RCC_SYSCLK_Div1);                            /* HCLK    = AHBCLK  = PLL / AHBPRES(1) = 144MHz.       */
    RCC_PCLK2Config(RCC_HCLK_Div2);                             /* APB2CLK = AHBCLK  / APB2DIV(2)       = 72MHz.        */
    RCC_PCLK1Config(RCC_HCLK_Div4);                             /* APB1CLK = AHBCLK  / APB1DIV(4)       = 36MHz         */
    
                                                                /* PLLCLK    = HSE * (PLLN / PLLM)      = 288MHz.       */
                                                                /* SYSCLK    = PLLCLK / PLLP            = 144MHz.       */
    ...
    RCC_SYSCLKConfig(RCC_SYSCLKSource_PLLCLK);                  /* HCLK = SYSCLK = PLL = 144MHz.                        */
    while (RCC_GetSYSCLKSource() != RCC_CFGR_SWS_PLL) {
        ;
    }
    
    BSP_LED_Init();                                             /* Init LEDs.                                           */
    BSP_ButtonInit();                                           /* Init user button                                     */
...
static void  BSP_LED_Init(void)
{
    ...
    gpio_init.GPIO_Pin   = BSP_GPIOD_LED4 | BSP_GPIOD_LED3 | BSP_GPIOD_LED5 | BSP_GPIOD_LED6;
...
*                       3    turns OFF user LD3
*                       4    turns OFF user LD4
*                       5    turns OFF user LD5
*                       6    turns OFF user LD6
*
* Return(s)   : none.
*
* Caller(s)   : Application.
...
void  BSP_LED_Toggle (CPU_INT08U  led)
{
    ...
    switch (led) {
        case 0u:
             ...
             pins ^= (BSP_GPIOD_LED3 | BSP_GPIOD_LED4 | BSP_GPIOD_LED5 | BSP_GPIOD_LED6);
...
*                                           BSP_ButtonRd()
*
* Description : Read the status of the STM32F4DISCOVERY's user button
*
* Argument(s) : none
*
* Return(s)   : DEF_TRUE    if the button is pressed
*               DEF_FALSE   otherwise
*
* Caller(s)   : Application.
*
...
CPU_BOOLEAN  BSP_ButtonRd (void)
{
    CPU_INT08U  button_val;
    
    
    button_val = GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_0);  
    if (button_val == 1) {
        return (DEF_TRUE);
    } else {
        return (DEF_FALSE);
