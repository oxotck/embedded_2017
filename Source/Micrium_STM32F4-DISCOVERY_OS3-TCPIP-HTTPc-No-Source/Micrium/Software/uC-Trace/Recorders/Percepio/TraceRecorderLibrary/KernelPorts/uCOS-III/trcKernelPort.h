 ...
 * trcKernelPort.h
 *
 * The defines for kernel functions used by the recorder and wrapper macros that uses the recorder macros in trcKernelHooks.h.
 * 
 * Terms of Use
...
#include <os.h>
#include <os_cfg.h>	                                            /* Defines TRACE_CFG_EN (1: Enabled).                  */
...
#define TRACE_CPU_CLOCK_HZ           TRACE_CFG_CPU_CLK_FREQ                             /* Peripheral clock speed in Hertz. */
#define TRACE_PERIPHERAL_CLOCK_HZ    TRACE_CFG_CPU_CLK_FREQ                             /* Peripheral clock speed in Hertz. */
...
#undef  TRACE_SR_ALLOC_CRITICAL_SECTION
...
#define TRACE_GET_MICRIUM_Q_NUMBER(p_q) (((OS_Q *)p_q)->MsgQID)
#define TRACE_SET_MICRIUM_Q_NUMBER(p_q) (((OS_Q *)p_q)->MsgQID = xTraceGetObjectHandle(TRACE_CLASS_QUEUE))
...
#define traceTASK_DELAY(dly_ticks) \
	...
	trcKERNEL_HOOKS_SET_TASK_INSTANCE_FINISHED(UNUSED, OSTCBCurPtr);
...
#define traceTASK_CREATE(p_tcb) \
	if (p_tcb != NULL) \
	{ \
		trcKERNEL_HOOKS_TASK_CREATE(CREATE, p_tcb); \
	}
...
#define traceTASK_CREATE_FAILED(p_tcb) \
	...
	trcKERNEL_HOOKS_TASK_CREATE_FAILED(CREATE, p_tcb); \
...
/* No need to protect critical section as it already is by the kernel. */
...
#define traceTASK_RESUME_FROM_ISR(p_tcb) \
	trcKERNEL_HOOKS_TASK_RESUME(TASK_RESUME_FROM_ISR, p_tcb);
