...
static  KAL_ERR   KAL_ErrConvert    (OS_ERR       err_os);
...
*
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_OS                  Concurrent initialization failed.
*                                   KAL_ERR_ALREADY_INIT        Already initialized and p_cfg is not NULL.
*                                   KAL_ERR_MEM_ALLOC           Memory segment allocation failed.
*                                   KAL_ERR_POOL_INIT           Memory pool initialization failed.
*
* Return(s)   : none.
*
                ...
                KAL_ERR  *p_err)
{
    ...
    if (KAL_IsInit == DEF_YES) {                                /* Chk if KAL already init. See note #1.                */
        ...
        if (init_status == KAL_INIT_STATUS_OK) {                /* Check resulting init status.                         */
            if (p_cfg == DEF_NULL) {
               *p_err = KAL_ERR_NONE;                           /* KAL_Init() can be called many times if no cfg.       */
            } else {
               *p_err = KAL_ERR_ALREADY_INIT;                   /* If a cfg is given and KAL is already init, set err.  */
            }
        } else {
           *p_err = KAL_ERR_OS;                                 /* Concurrent init failed.                              */
        }
        return;
    }
...
*
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_INVALID_ARG         Specified stack size is invalid.
*                                   KAL_ERR_UNIMPLEMENTED       'p_cfg' parameter was not NULL.
*                                   KAL_ERR_MEM_ALLOC           Task stack or control block allocation failed.
*
...
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_UNIMPLEMENTED       'p_cfg' parameter was not NULL.
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Invalid argument passed to function.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*
* Return(s)   : none.
*
* Caller(s)   : Micrium product(s).
...
*
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Invalid argument passed to function.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*                                   KAL_ERR_OS                  Generic OS error.
*
* Return(s)   : none.
*
* Caller(s)   : Micrium product(s).
...
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_MEM_ALLOC           Unable to allocate memory for lock.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*                                   KAL_ERR_INVALID_ARG         Invalid argument passed to function.
*
...
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Handle or options specified is invalid.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*                                   KAL_ERR_ABORT               Pend operation was aborted.
*                                   KAL_ERR_TIMEOUT             Operation timed-out.
*                                   KAL_ERR_LOCK_OWNER          Calling task already owns the lock.
*                                   KAL_ERR_WOULD_BLOCK         KAL_OPT_PEND_NON_BLOCKING opt specified and
...
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Task releasing lock does not own lock.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*
* Return(s)   : none.
*
...
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Handle or options specified is invalid.
*                                   KAL_ERR_ISR                 Function was called from an ISR.
*                                   KAL_ERR_OS                  Generic OS error.
*
* Return(s)   : none.
*
...
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_UNIMPLEMENTED       'p_cfg' parameter was not NULL.
*                                   KAL_ERR_MEM_ALLOC           Unable to allocate memory for semaphore.
*                                   KAL_ERR_INVALID_ARG         Invalid argument passed to function.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*
...
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Handle or options specified is invalid.
*                                   KAL_ERR_OVF                 Semaphore would overflow.
*
* Return(s)   : none.
*
* Caller(s)   : Micrium product(s).
...
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Handle specified is invalid.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*
* Return(s)   : none.
*
...
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Null pointer passed to function.
*                                   KAL_ERR_MEM_ALLOC           Unable to allocate memory for semaphore.
*                                   KAL_ERR_INVALID_ARG         Invalid argument passed to function.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*                                   KAL_ERR_OS                  Generic OS error.
                                      ...
                                      KAL_ERR           *p_err)
{
    ...
    #if (OS_CFG_TMR_EN == DEF_ENABLED)
    {
        ...
        if ((p_cfg                                            != DEF_NULL) &&
            ...
            tmr_dly    = 0u;
            tmr_period = KAL_msToTicks(interval_ms);            /* Calc nbr of tmr ticks (rounded up nearest int).      */
        } else {
            opt_os     = OS_OPT_TMR_ONE_SHOT;                   /* Tmr is 'one-shot'.                                   */
            tmr_dly    = KAL_msToTicks(interval_ms);            /* Calc nbr of tmr ticks (rounded up nearest int).      */
            tmr_period = 0u;
        }
...
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Null timer handle passed to function.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*                                   KAL_ERR_OS                  Generic OS error.
*
* Return(s)   : none.
...
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_UNIMPLEMENTED       'p_cfg' parameter was not NULL.
*                                   KAL_ERR_MEM_ALLOC           Unable to allocate memory for queue.
*                                   KAL_ERR_ISR                 Function called from an ISR context.
*                                   KAL_ERR_INVALID_ARG         Argument passed to function is invalid.
*
...
*                                   KAL_ERR_TIMEOUT             Operation timed-out.
*                                   KAL_ERR_ISR                 Function was called from an ISR.
*                                   KAL_ERR_WOULD_BLOCK         KAL_OPT_PEND_NON_BLOCKING opt specified and no
...
*                                   KAL_ERR_NULL_PTR            Handle contains a NULL/invalid pointer.
*                                   KAL_ERR_INVALID_ARG         Handle or options specified is invalid.
*                                   KAL_ERR_RSRC                Queue cannot contain any more message,
...
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_UNIMPLEMENTED       'p_cfg' parameter was not NULL.
*                                   KAL_ERR_MEM_ALLOC           Unable to allocate memory for task register.
*                                   KAL_ERR_RSRC                No more task register available.
*
...
*                                   KAL_ERR_NONE                No error.
*                                   KAL_ERR_NOT_AVAIL           Configuration does not allow operation.
*                                   KAL_ERR_NULL_PTR            Task register handle is NULL.
*                                   KAL_ERR_INVALID_ARG         Handle specified is invalid.
*
...
static  KAL_ERR  KAL_ErrConvert (OS_ERR  err_os)
{
    ...
    switch (err_os) {
        ...
        case OS_ERR_TMR_INVALID_PERIOD:
             err_kal = KAL_ERR_INVALID_ARG;
             break;


        case OS_ERR_OBJ_PTR_NULL:
        case OS_ERR_PTR_INVALID:
             err_kal = KAL_ERR_NULL_PTR;
             break;
        ...
        case OS_ERR_TIMEOUT:
             err_kal = KAL_ERR_TIMEOUT;
             break;


        case OS_ERR_PEND_ABORT:
             err_kal = KAL_ERR_ABORT;
             break;
        ...
        case OS_ERR_Q_MAX:
             err_kal = KAL_ERR_RSRC;
             break;


        case OS_ERR_MUTEX_OWNER:
             err_kal = KAL_ERR_LOCK_OWNER;
             break;
